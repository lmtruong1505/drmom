...
@Injectable()
class StoreAddressSelectionCubit extends BaseCubit<StoreAddressSelectionState> {
  StoreAddressSelectionCubit(this._baseDio)
      : super(const StoreAddressSelectionState());
  ...
  void handleSearch(String keyword) {
    ...
    if (keyword.isNotEmpty) {
      final searchs = dataClone.where((e) {
        final name =
            removeVietnameseTones((e['title'] as String).toLowerCase());
  ...
  void onChangeWard(dynamic ward, {bool? hiddenDetail = false}) {
    ...
    if (state.district != null) {
      text.add(state.district['title']);
    }
    if (state.province != null) {
      text.add(state.province['title']);
    }
