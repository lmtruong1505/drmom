...
@RoutePage()
class OtpVerificationTokenPage extends StatefulWidget {
  final String token;

  const OtpVerificationTokenPage({
    Key? key,
    required this.token,
  }) : super(key: key);

  @override
  State<OtpVerificationTokenPage> createState() =>
      _OtpVerificationTokenPageState();
}

class _OtpVerificationTokenPageState extends State<OtpVerificationTokenPage> {
  @override
  void initState() {
    super.initState();
    bloc.startTimer();
          ...
          body: Container(
            ...
            child: Column(
              ...
              children: [
                ...
                //           Expanded(
                //             child: Text(
                //               preferences.currentUser.phoneNumber ?? "",
                            ...
                            builder: (context, state) {
                              return Center(
                                child: Column(
                                  children: [
                                    Text(
                                      "Mã xác thực được gửi đến SĐT ${preferences.currentUser.phoneNumber}",
                                      style: AppTypography.p5,
                                    ),
                                    const Text(
                ...
                BlocBuilder<AuthenticationCubit, AuthenticationState>(
                  builder: (context, state) {
                    return SizedBox(
                      ...
                      child: MainButton(
                        title: 'Xác nhận',
                        onTap: () => bloc.onChangeToken(widget.token),
